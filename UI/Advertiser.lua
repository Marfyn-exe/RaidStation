...
function AdvertiserUI.CreatePanel(parent)
    ...
    local function syncRaidMarks(oldStr, newStr)
        if not oldStr or oldStr == "" then return newStr end
        if not oldStr:find("{rt%d}") then return newStr end

        -- Extraer marcas y sus posiciones en texto plano limpio
        local savedMarks = {}
        local cleanChars = {}
        local i = 1
        local lenOldFull = #oldStr
        local plainIdx = 1

        while i <= lenOldFull do
            if i + 4 <= lenOldFull and oldStr:sub(i, i + 4):match("{rt%d}") then
                local mark = oldStr:sub(i, i + 4)
                table.insert(savedMarks, { pos = plainIdx, token = mark })
                i = i + 5
            else
                cleanChars[#cleanChars + 1] = oldStr:sub(i, i)
                plainIdx = plainIdx + 1
                i = i + 1
            end
        end
        local oldClean = table.concat(cleanChars)
        local newClean = stripRT(newStr or "")

        local lenOld, lenNew = #oldClean, #newClean
        local lcp = 0
        while lcp < lenOld and lcp < lenNew do
            if oldClean:sub(lcp + 1, lcp + 1) == newClean:sub(lcp + 1, lcp + 1) then
                lcp = lcp + 1
            else
                break
            end
        ...
        while lcs < maxLcs do
            local oldChar = oldClean:sub(lenOld - lcs, lenOld - lcs)
            local newChar = newClean:sub(lenNew - lcs, lenNew - lcs)
        ...
        for idx = 1, lenNew + 1 do
            ...
            if idx <= lenNew then
                finalChars[#finalChars + 1] = newClean:sub(idx, idx)
            end
        end
    ...
    function AdvertiserUI:ActualizarHeader(isSilent)
        ...
        -- Guardar el header ANTERIOR antes de sobreescribirlo
        local oldAutoGenerated = p.lastAutoGenerated or ""

        -- Variables de control (actualizar DESPUÉS de guardar el viejo)
        ...
        -- Split currentFull into zones using the OLD auto-generated header
        -- (el viejo es el que realmente está dentro de currentFull)
        local oldExtra = ""
        local oldNotes = ""
        local found = false

        -- Primero intentar con el header ANTERIOR (lo que realmente contiene currentFull)
        if oldAutoGenerated ~= "" then
            local autoStart, autoEnd = findIgnoringRT(currentFull, oldAutoGenerated)
            if autoStart and autoEnd then
                oldExtra = currentFull:sub(1, autoStart - 1)
                oldNotes = currentFull:sub(autoEnd + 1)
                found = true
            end
        end

        -- Si no encontró con el viejo, intentar con el nuevo (por si son iguales o edición menor)
        if not found and newAuto ~= "" then
            local autoStart, autoEnd = findIgnoringRT(currentFull, newAuto)
            if autoStart and autoEnd then
                oldExtra = currentFull:sub(1, autoStart - 1)
                oldNotes = currentFull:sub(autoEnd + 1)
                found = true
        ...
        if not found then
            -- Fallback si no encontramos la cabecera anterior (ej: edición total)
            oldExtra = p.extraHeader or ""
            -- Evitar usar currentFull completo como notas (causa duplicación de marcas).
            -- Intentar quitar el extraHeader conocido del inicio.
            if oldExtra ~= "" and currentFull:sub(1, #oldExtra) == oldExtra then
                oldNotes = currentFull:sub(#oldExtra + 1)
            else
                oldNotes = currentFull
            end
        end

        -- Reconstruir notas con marcas preservadas
        local newNotesPart = (newNotes ~= "") and (" " .. newNotes) or ""
        local finalNotes = syncRaidMarks(oldNotes, newNotesPart)

        -- Reconstruir mensaje completo
        local txt = oldExtra .. newAuto .. finalNotes
        p.fullMessage = txt
