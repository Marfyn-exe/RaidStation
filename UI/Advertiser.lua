...
function AdvertiserUI.CreatePanel(parent)
    ...
    function AdvertiserUI:ActualizarHeader(isSilent)
        local p = ns.Advertiser.patterns
        local newAuto = ns.Advertiser:GetFormattedMessage() or ""
        ...
        -- 3. Identificar la ubicación de las notas viejas de forma inteligente
        -- Buscaremos la porción final después de la lista de roles fijas: "Caster (d) [0/10]"
        local anchorPattern = "%]%s*"
        local anchorStart, anchorEnd = currentPlain:find(anchorPattern)
        
        local beforeRoles = currentPlain
        if anchorEnd then
            beforeRoles = currentPlain:sub(1, anchorEnd)
        end
        ...
        -- 4. Re-inyectar las marcas basándonos en la estabilidad de los extremos
        local finalChars = {}
        local markMap = {}
        local oldPlainLen = #currentPlain
        local newPlainLen = #newPlain

        for _, m in ipairs(savedMarks) do
            local targetPos = m.pos
            if m.pos > oldPlainLen then
                -- Si estaba al final del todo (ej. las calaveras en las notas), va al final del nuevo string
                targetPos = newPlainLen + 1
            elseif anchorEnd and m.pos > anchorEnd then
                -- Si la marca estaba en la sección de notas, se ajusta proporcionalmente al tamaño de la nueva nota
                local notesOffset = m.pos - anchorEnd
                local oldNotesLen = oldPlainLen - anchorEnd
                if oldNotesLen > 0 then
                    local ratio = notesOffset / oldNotesLen
                    local newNotesLen = newPlainLen - anchorEnd
                    targetPos = anchorEnd + math.max(0, math.floor(ratio * newNotesLen + 0.5))
                else
                    targetPos = newPlainLen + 1
                end
            else
                -- Si estaba en el encabezado o roles, se mantiene proporcional al cambio del bloque autogenerado
                if anchorEnd and anchorEnd > 0 then
                    local ratio = m.pos / anchorEnd
                    targetPos = math.max(1, math.floor(ratio * #cleanNewAuto + 0.5))
                end
            end
            
            -- Asegurar límites correctos
            targetPos = math.max(1, math.min(newPlainLen + 1, targetPos))
            markMap[targetPos] = (markMap[targetPos] or "") .. m.token
        end

        -- Ensamblado final intercalado
        for idx = 1, newPlainLen + 1 do
            if markMap[idx] then
                finalChars[#finalChars + 1] = markMap[idx]
            end
            if idx <= newPlainLen then
    ...
    function AdvertiserUI:RefreshAllInputs()
        ...
        if p.fullMessage and p.fullMessage ~= "" then
            self.previewEdit:SetText(p.fullMessage)
            p.lastAutoGenerated = ns.Advertiser:GetFormattedMessage() or ""
            ...
            p.lastSyncNotes = ""
            p.fullMessage = base    -- fix del bug anterior
