...
function AdvertiserUI.CreatePanel(parent)
    ...
    function AdvertiserUI:ActualizarHeader(isSilent)
        ...
        -- Split currentFull into zones based on p.lastAutoGenerated
        local oldExtra = ""
        local oldNotes = ""
        local found = false

        if p.lastAutoGenerated and p.lastAutoGenerated ~= "" then
            local autoStart, autoEnd = findIgnoringRT(currentFull, p.lastAutoGenerated)
